# Report — findings and decision framework

Every number and table below is generated by `make report` from `results/runs/*.json` and
`results/quality/*.json`. Nothing here is hand-typed; rebuilding on unchanged results leaves
`git diff` empty. See [METHODOLOGY.md](METHODOLOGY.md) for how the measurements are produced
and which of them are not trustworthy.

**What was measured.** Every run below is on one NVIDIA A40 (sm_86) serving
`meta-llama/Llama-3.1-8B-Instruct` at `max_model_len=4096`: open-loop Poisson arrivals seeded
identically across configurations, clocks locked and verified against sampled telemetry, kernel
selection asserted from engine startup logs, plus the runs that test the method itself (§6).

<!-- BEGIN:headline -->
**244 measured runs** across 6 configurations, 6 quality evaluations, open-loop, at least three repeats per point.

Under a **p95 TTFT budget of 500 ms**, `vllm-int8-w8a8` is cost-optimal: **2116 output tokens/sec at $0.0525 per million tokens** — 27 % cheaper than `vllm-bf16`. `vllm-bf16` is dominated: 4 configurations are cheaper at equal-or-better measured quality.

The grey band on the chart is `vllm-bf16`'s 95 % confidence interval on GSM8K, and every configuration falls inside it. The quality spread across all 6 configurations is 0.0159, smaller than a single configuration's 95 % half-width of 0.0240 — **quality does not separate these configurations at this sample size**, so the decision is made on cost and latency.
<!-- END:headline -->

---

## 1. Headline — the Pareto frontier

![Quality vs cost at a fixed latency budget](results/figures/pareto_quality_cost.png)

**Read the grey band before reading the ranking.** Any claim that one configuration is *more
accurate* than another here would be reading noise. What the chart shows is that quality does
not separate these configurations, so the decision collapses onto cost and latency — where the
differences are large and reproducible. The unquantized baseline is on nobody's frontier.

<!-- BEGIN:cost-note -->
Cost assumes **$0.40/GPU-hour** (RunPod, https://www.runpod.io/pricing, accessed 2026-08-10). The benchmark GPU is a lab machine with no invoice, so this is an assumption. Because every configuration runs on the same GPU, the hourly rate is a linear scalar and the *ranking* is invariant to it — there is no crossover price.
<!-- END:cost-note -->

---

## 2. Finding 1 — decode time tracks weight bytes, not FLOPs

The premise of the whole quantization axis was that A40 decode is memory-bandwidth-bound. If
that is true, shrinking the weights should speed up decode in proportion to the bytes no longer
read. Measured at 1 req/s, where batching is minimal and the bandwidth bound is cleanest:

<!-- BEGIN:bandwidth-table -->
| Config | Weights | Predicted speedup (byte ratio) | Measured TPOT p95 | Measured speedup |
|---|---|---|---|---|
| `vllm-bf16` | 15.00 GiB | 1.00x | 32.56 ms | 1.00x |
| `vllm-int8-w8a8` | 8.50 GiB | 1.76x | 18.59 ms | **1.75x** |
| `vllm-gptq-int4` | 5.70 GiB | 2.63x | 10.96 ms | 2.97x |
| `vllm-awq-int4` | 5.70 GiB | 2.63x | 10.33 ms | 3.15x |

Act-order overhead, isolated: `gptq_marlin` 10.96 ms vs `awq_marlin` 10.33 ms — **a 6.2 % decode penalty** for `desc_act=true` at identical bit width.
<!-- END:bandwidth-table -->

**INT8 lands on the prediction within about 1 %.** That is the cleanest test in the table, because
W8A8 changes weight *and* activation precision together and leaves no dequantization step to
confound the timing. Decode on this GPU is bandwidth-bound, and the bytes-read model predicts
it quantitatively rather than directionally.

**The INT4 configurations beat the prediction, and the bandwidth model does not explain that.**
A pure bytes-read model cannot produce a speedup larger than the byte ratio — any fixed
per-token overhead pushes the measured ratio *below* it, not above. The likely cause is that
the baseline is not itself bandwidth-optimal: Marlin is specifically tuned for the low-batch
regime, while vLLM's general BF16 GEMM path is not, so part of the INT4 speedup beyond 2.63× is
kernel quality rather than byte count. **This benchmark does not isolate that**, and the number is reported as
unexplained rather than claimed as confirmation.

### Act-order overhead, isolated

Both INT4 checkpoints are 4-bit, group size 128, and both select a Marlin kernel. They differ
in that the GPTQ checkpoint carries `desc_act=true`, which `gptq_marlin` handles with a
load-time permutation; AWQ-GEMM has no act-order concept. The pair therefore isolates that
overhead rather than duplicating a data point — the penalty is the last line of the table
above. Under saturation it compounds rather than staying constant (§3).

---

## 3. Finding 2 — the best quantization inverts between load regimes

This is the result that would be missed by any benchmark reporting a single throughput number.

<!-- BEGIN:regime-table -->
| Offered rate | TPOT p95 winner | TTFT p95 winner | Configs still valid |
|---|---|---|---|
| 1 rps | `vllm-awq-int4` (10.3 ms) | `vllm-int8-w8a8` (97 ms) | 4 of 4 |
| 2 rps | `vllm-awq-int4` (11.5 ms) | `vllm-awq-int4` (87 ms) | 4 of 4 |
| 4 rps | `vllm-awq-int4` (16.2 ms) | `vllm-int8-w8a8` (160 ms) | 4 of 4 |
| 5 rps | `vllm-awq-int4` (19.9 ms) | `vllm-int8-w8a8` (208 ms) | 4 of 4 |
| 6 rps | `vllm-awq-int4` (25.4 ms) | `vllm-int8-w8a8` (276 ms) | 4 of 4 |
| 7 rps | `vllm-int8-w8a8` (47.4 ms) | `vllm-int8-w8a8` (330 ms) | 4 of 4 |
| 8 rps | `vllm-int8-w8a8` (66.7 ms) | `vllm-int8-w8a8` (459 ms) | 3 of 4 |
| 9 rps | `vllm-int8-w8a8` (102.0 ms) | `vllm-int8-w8a8` (2353 ms) | 1 of 4 |
<!-- END:regime-table -->

INT4 holds the decode advantage through 6 rps. At 7 rps INT8 overtakes it on decode as well as
prefill, and at 8 req/s the INT4 configurations do not merely lose — they collapse. TTFT p95
goes to **3288 ms for AWQ and 9594 ms for GPTQ, against 459 ms for INT8**, a 7× and 21× gap
against a configuration that was *behind* on decode latency at every rate below the knee. BF16 does not reach
8 rps at all; every repeat was classified `oversubscribed`.

**The mechanism is the activation precision, not the weight precision.** Both INT4 formats here
are W4A16: 4-bit weights, 16-bit activations. The GEMM dequantizes weights to FP16 and does
FP16 math, so INT4 buys bandwidth and nothing else. That is exactly right for decode, which is
bandwidth-bound — and worth nothing for prefill, which is compute-bound, where it additionally
pays for the dequantization. W8A8 is different in kind: it feeds the A40's INT8 tensor cores
(~299 TOPS against ~149.7 TFLOPS BF16), so it accelerates prefill *arithmetic*. As offered load
rises, prefill work grows, the bottleneck migrates from bandwidth to compute, and the
configuration that only ever addressed bandwidth runs out of room.

GPTQ collapsing harder than AWQ (9594 vs 3288 ms) is the act-order penalty compounding: a
6.2 % per-token cost at low load becomes a queueing catastrophe once the server is at capacity,
because the service rate falls below the arrival rate and the queue grows without bound.

**Practical consequence.** "Which quantization is fastest" has no load-independent answer on
this hardware. Below the knee, pick for decode bandwidth: INT4. At or near the knee, pick for
prefill compute: INT8. A benchmark that reports one rate cannot see this.

---

## 4. Decision framework — cost-optimal configuration vs SLA

Given a p95 TTFT budget, the cheapest configuration that meets it:

<!-- BEGIN:sla-table -->
| p95 TTFT budget | Cheapest config | Rate | Throughput | $/1M tokens | Admissible |
|---|---|---|---|---|---|
| 200 ms | `vllm-awq-int4` | 4 rps | 1112 tok/s | $0.0999 | 6 of 6 |
| 500 ms | `vllm-int8-w8a8` | 8 rps | 2116 tok/s | $0.0525 | 6 of 6 |
| 1000 ms | `vllm-int8-w8a8` | 8 rps | 2116 tok/s | $0.0525 | 6 of 6 |
| 2000 ms | `vllm-int8-w8a8` | 8 rps | 2116 tok/s | $0.0525 | 6 of 6 |
<!-- END:sla-table -->

The crossover at the tight end is real: at a 200 ms budget no configuration sustains its
highest rate, so the ranking is decided by which one still meets the budget at 4 rps, and
`vllm-awq-int4` wins on cost there. From 500 ms upward `vllm-int8-w8a8` wins, and the answer
stops changing — not because the budget stops mattering, but because the measured ladder tops
out at 8 rps. **Above roughly 500 ms the binding constraint is engine capacity, not the SLA**,
and a further-loosened budget buys nothing. Extending the ladder past 8 rps for INT8 would be
the natural next measurement; every rate above it in the current matrix is `oversubscribed`
and therefore unreportable.

### How this transfers off the A40

The mechanism generalizes even though the numbers do not. The question to ask of any
accelerator is where its arithmetic intensity sits relative to the model's: the A40 is
compute-rich and bandwidth-poor (~215 FLOP/byte against ~153 for an A100-80GB), which is why
weight-only quantization pays here more than published A100 results suggest, and why the
prefill regime flips to favouring a format with real low-precision compute. On a part with FP8
tensor cores (sm_89/sm_90) the W8A8 role would be filled by FP8 rather than INT8 — which is
precisely why FP8 was excluded here rather than faked (§7).

---

## 5. Results

### Operating points

Only `valid` runs appear. Oversubscribed runs are excluded on purpose: past capacity, latency
is a function of how long the run lasted rather than of offered load, so an oversubscribed
point is not an operating point anyone could provision for.

<!-- BEGIN:latency-table -->
| Config | Rate | Throughput | TTFT p95 | TPOT p95 | $/1M tokens | Repeats |
|---|---|---|---|---|---|---|
| `sglang-awq-int4` | 1 rps | 287 ± 0 tok/s | 122 ms | 10.7 ms | $0.3878 | 3 |
| `sglang-awq-int4` | 2 rps | 606 ± 0 tok/s | 106 ms | 12.5 ms | $0.1834 | 3 |
| `sglang-awq-int4` | 4 rps | 1112 ± 0 tok/s | 217 ms | 18.6 ms | $0.0999 | 3 |
| `sglang-awq-int4` | 5 rps | 1376 ± 4 tok/s | 362 ms | 47.9 ms | $0.0807 | 3 |
| `sglang-awq-int4` | 6 rps | 1631 ± 5 tok/s | 486 ms | 60.1 ms | $0.0681 | 3 |
| `sglang-awq-int4` | 7 rps | 1866 ± 5 tok/s | 570 ms | 84.9 ms | $0.0595 | 3 |
| `sglang-awq-int4` | 8 rps | 2054 ± 8 tok/s | 1465 ms | 120.3 ms | $0.0541 | 3 |
| `sglang-bf16` | 1 rps | 265 ± 0 tok/s | 143 ms | 33.6 ms | $0.4197 | 3 |
| `sglang-bf16` | 2 rps | 564 ± 1 tok/s | 142 ms | 37.6 ms | $0.1969 | 3 |
| `sglang-bf16` | 4 rps | 1026 ± 0 tok/s | 224 ms | 51.6 ms | $0.1083 | 3 |
| `sglang-bf16` | 5 rps | 1282 ± 0 tok/s | 304 ms | 61.5 ms | $0.0867 | 3 |
| `sglang-bf16` | 6 rps | 1536 ± 0 tok/s | 392 ms | 70.0 ms | $0.0724 | 3 |
| `sglang-bf16` | 7 rps | 1761 ± 2 tok/s | 468 ms | 94.6 ms | $0.0631 | 3 |
| `sglang-bf16` | 8 rps | 1965 ± 1 tok/s | 2670 ms | 118.4 ms | $0.0565 | 3 |
| `vllm-awq-int4` | 1 rps | 286 ± 0 tok/s | 104 ms | 10.3 ms | $0.3878 | 3 |
| `vllm-awq-int4` | 2 rps | 606 ± 0 tok/s | 87 ms | 11.5 ms | $0.1834 | 3 |
| `vllm-awq-int4` | 4 rps | 1112 ± 0 tok/s | 194 ms | 16.2 ms | $0.0999 | 3 |
| `vllm-awq-int4` | 5 rps | 1390 ± 0 tok/s | 268 ms | 19.9 ms | $0.0799 | 3 |
| `vllm-awq-int4` | 6 rps | 1675 ± 0 tok/s | 363 ms | 25.4 ms | $0.0663 | 3 |
| `vllm-awq-int4` | 7 rps | 1912 ± 0 tok/s | 468 ms | 54.5 ms | $0.0581 | 3 |
| `vllm-awq-int4` | 8 rps | 2035 ± 2 tok/s | 3288 ms | 129.2 ms | $0.0546 | 3 |
| `vllm-bf16` | 1 rps | 265 ± 0 tok/s | 168 ms | 32.6 ms | $0.4197 | 6 |
| `vllm-bf16` | 2 rps | 565 ± 1 tok/s | 152 ms | 36.0 ms | $0.1965 | 6 |
| `vllm-bf16` | 4 rps | 1052 ± 34 tok/s | 263 ms | 45.4 ms | $0.1056 | 11 |
| `vllm-bf16` | 5 rps | 1284 ± 0 tok/s | 364 ms | 55.6 ms | $0.0865 | 6 |
| `vllm-bf16` | 6 rps | 1539 ± 0 tok/s | 481 ms | 66.1 ms | $0.0722 | 6 |
| `vllm-bf16` | 7 rps | 1748 ± 1 tok/s | 678 ms | 109.5 ms | $0.0636 | 6 |
| `vllm-gptq-int4` | 1 rps | 286 ± 0 tok/s | 104 ms | 11.0 ms | $0.3886 | 3 |
| `vllm-gptq-int4` | 2 rps | 605 ± 0 tok/s | 89 ms | 12.2 ms | $0.1836 | 3 |
| `vllm-gptq-int4` | 4 rps | 1110 ± 0 tok/s | 198 ms | 17.2 ms | $0.1001 | 3 |
| `vllm-gptq-int4` | 5 rps | 1386 ± 0 tok/s | 301 ms | 21.3 ms | $0.0802 | 3 |
| `vllm-gptq-int4` | 6 rps | 1670 ± 0 tok/s | 379 ms | 27.5 ms | $0.0665 | 3 |
| `vllm-gptq-int4` | 7 rps | 1897 ± 1 tok/s | 512 ms | 62.4 ms | $0.0586 | 3 |
| `vllm-gptq-int4` | 8 rps | 1972 ± 5 tok/s | 9594 ms | 137.9 ms | $0.0563 | 3 |
| `vllm-int8-w8a8` | 1 rps | 277 ± 0 tok/s | 97 ms | 18.6 ms | $0.4005 | 3 |
| `vllm-int8-w8a8` | 2 rps | 596 ± 0 tok/s | 98 ms | 21.1 ms | $0.1863 | 3 |
| `vllm-int8-w8a8` | 4 rps | 1078 ± 0 tok/s | 160 ms | 26.7 ms | $0.1031 | 3 |
| `vllm-int8-w8a8` | 5 rps | 1347 ± 0 tok/s | 208 ms | 31.8 ms | $0.0825 | 3 |
| `vllm-int8-w8a8` | 6 rps | 1620 ± 0 tok/s | 276 ms | 37.6 ms | $0.0686 | 3 |
| `vllm-int8-w8a8` | 7 rps | 1877 ± 0 tok/s | 330 ms | 47.4 ms | $0.0592 | 3 |
| `vllm-int8-w8a8` | 8 rps | 2116 ± 1 tok/s | 459 ms | 66.7 ms | $0.0525 | 3 |
| `vllm-int8-w8a8` | 9 rps | 2303 ± 6 tok/s | 2353 ms | 102.0 ms | $0.0482 | 3 |
<!-- END:latency-table -->

### Quality

Greedy decoding, fixed seed, identical prompts, measured **through the serving engine** so the
quantized kernels are the ones actually exercised. GSM8K is 8-shot on the completions endpoint;
IFEval is 0-shot with the chat template applied.

<!-- BEGIN:quality-table -->
| Config | Weights | WikiText-2 PPL | ΔPPL | GSM8K | Δ | IFEval | Δ |
|---|---|---|---|---|---|---|---|
| `vllm-bf16` | 15.0 GiB | 6.3545 | +0.0000 | 0.7293 | +0.0000 | 0.7449 | +0.0000 |
| `vllm-int8-w8a8` | 8.5 GiB | 6.4400 | +0.0855 | 0.7445 | +0.0152 | 0.7579 | +0.0129 |
| `vllm-awq-int4` | 5.7 GiB | 6.6619 | +0.3074 | 0.7316 | +0.0023 | 0.7449 | +0.0000 |
| `vllm-gptq-int4` | 5.7 GiB | 6.8642 | +0.5098 | 0.7407 | +0.0114 | 0.7375 | -0.0074 |
<!-- END:quality-table -->

**Perplexity separates these configurations; the task benchmarks do not.** WikiText-2 perplexity
orders them monotonically by bit width, with a spread well outside measurement noise. GSM8K and
IFEval place all four within each other's confidence intervals. This is not a defect of the
tasks — it is a statement about statistical power: at ~1300 GSM8K samples the resolvable
difference is a few points, and quantization damage here is smaller than that. Reporting the
task scores as a quality *ranking* would be overclaiming; they are reported as evidence that no
configuration is grossly broken, which is what they can actually support.

### Measurement validity

<!-- BEGIN:validity -->
| Validity | Runs | Meaning |
|---|---|---|
| `valid` | 187 | reportable |
| `oversubscribed` | 57 | offered load beyond capacity; latency reflects run duration |
<!-- END:validity -->

No run was discarded for client saturation, and no run throttled. The `oversubscribed`
records are kept and published rather than deleted: which offered rates a configuration
*cannot* serve is a finding about its capacity.

---

## 6. Does the methodology hold up?

The rest of this report rests on three methodological claims. Each is testable,
and each has a run whose only purpose is to test it rather than to rank a configuration.
None of these appear on the frontier: a trace replay and a closed-loop run carry no offered
rate, which is the field every aggregation keys on, so they are structurally excluded from
the operating points above.

### The arrival process — what a closed loop gets wrong, and what Poisson costs

Both exhibits face the same server and the same prompts as the open-loop baseline; only the
arrival process differs. The closed-loop pool size is swept so that a throughput-matched point
exists for each open-loop rate, because a tail comparison across different loads measures the
load, not the generator. The trace replay is five independent non-overlapping windows against
five independent Poisson seeds, interleaved, because one seeded schedule repeated three times
measures the server's variability rather than the arrival process's.

<!-- BEGIN:methodology-table -->
| Arrival process | Matched to | TTFT p50 | TTFT p95 | TTFT p99 | Throughput | Tail vs open-loop Poisson |
|---|---|---|---|---|---|---|
| **Open-loop Poisson** (baseline) | 4 rps | 160 ms | 304 ms | 538 ms | 1082 tok/s | — |
| Azure trace replay | 4 rps | 159 ms | 313 ms | 542 ms | 1056 tok/s | indistinguishable (+4 ms against ±21 ms) |
| Closed loop (N=8) | 1 rps (250 ms p99) | 91 ms | 199 ms | 402 ms | 252 tok/s | **1.6x worse** (p99 +151 ms) |
| Closed loop (N=48) | 4 rps (538 ms p99) | 153 ms | 294 ms | 543 ms | 1040 tok/s | indistinguishable (+4 ms against ±21 ms) |
| Closed loop (N=64) | 5 rps (623 ms p99) | 171 ms | 359 ms | 2159 ms | 1211 tok/s | **3.5x worse** (p99 +1536 ms) |
| Closed loop (N=96) | 6 rps (714 ms p99) | 213 ms | 680 ms | 2672 ms | 1563 tok/s | **3.7x worse** (p99 +1957 ms) |
| Closed loop (N=128) | 7 rps (988 ms p99) | 261 ms | 2348 ms | 3308 ms | 1733 tok/s | **3.3x worse** (p99 +2320 ms) |
<!-- END:methodology-table -->

**This repository's own premise was wrong in direction, and the exhibit is what showed it.**
The textbook objection to closed-loop generators is *coordinated omission*: when the server
stalls, a generator that waits for completions stops sampling, so the slow periods are
under-represented and the reported tail is optimistic. That was the claim here, and it
predicted that the closed-loop rows would *understate* the open-loop p99.

Measured, they never do. Below the knee the two generators are indistinguishable; at and above
it the closed loop reports a tail **three to four times worse** than open-loop Poisson at the
same throughput. The mechanism is not coordinated omission but its opposite. A continuous-
batching engine below saturation has no stalls to hide, so the classic error has nothing to
act on. What the closed loop does instead is pin occupancy at a constant maximum: every new
request arrives into a server already running N−1 others, so its prefill always competes with
a full batch of decodes. Poisson arrivals at the same mean let occupancy fluctuate, and the
requests that arrive into a lull get fast prefill. The closed loop cannot produce a lull.

Open-loop remains the correct methodology — but for a reason the measurement had to supply.
The argument is not that a closed loop flatters the server; on this stack it slanders it.
The argument is that a closed loop's load is a *consequence of the server's speed*, so it
cannot measure the server against any load a real deployment would actually receive. Both
errors are the same error: the generator and the thing under test are coupled.

The near-knee closed-loop rows (N = 96, 128) were measured on GPU 0 against an open-loop ladder
measured on GPU 1. Both cards are the same model at the same locked clock, and the 4 rps row —
measured on the same card as its baseline — shows the same null result below the knee, so the
card cannot be what produces the sign at the knee. It is stated rather than hidden.

**The Poisson assumption costs nothing measurable here.** The trace replay sits +4 ms above
Poisson at p99, against a ±21 ms interval. This agrees with two properties of the trace itself:
its inter-arrival CV² is 1.02 (Poisson: 1.00), and its count dispersion is ≈1.0 at every
timescale from 0.5 s to 30 s. For this workload at this rate, Poisson is not an assumption; it
is a measured property of the traffic.

### Environmental drift across the sweep

The first configuration re-run at the end, identical in every respect. If it agrees with the
original within the sweep's own repeat-to-repeat noise, results measured hours apart are
comparable; if it does not, every cross-configuration claim above is weakened.

<!-- BEGIN:drift-table -->
| | First measurement | Re-run at end of sweep | Δ |
|---|---|---|---|
| TTFT p95 | 252.6 ms | 266.7 ms | +14.1 ms |
| Throughput | 1028 tok/s | 1062 tok/s | +3.2 % |

Repeat-to-repeat std of the original: ±36.7 ms. The re-run is within noise — environmental drift across the sweep is bounded.
<!-- END:drift-table -->

### Agreement with an independent harness

Every other correctness check in this repository is self-referential — the percentile function
against numpy, the arrival process against a KS test, TTFT against internal consistency. All of
it can be true while the harness measures the wrong thing consistently.

<!-- BEGIN:crossvalidation-table -->
**Matched workload** — `vllm-bf16` at 4 rps, both harnesses on constant 256/256 input/output tokens against the same live server:

| Metric | This harness | `vllm bench serve` | Δ | Agrees (±5 %) |
|---|---|---|---|---|
| TTFT mean | 174.32 ms | 168.38 ms | +3.5 % | yes |
| TTFT p99 | 273.24 ms | 260.50 ms | +4.9 % | yes |
| TPOT mean | 40.62 ms | 41.72 ms | -2.6 % | yes |
| TPOT p99 | 45.18 ms | 45.90 ms | -1.6 % | yes |
| E2E mean | 10531.72 ms | 10805.71 ms | -2.5 % | yes |
| Output throughput | 927.64 tok/s | 977.33 tok/s | -5.1 % | no* |

**Unmatched workload** — the same comparison with each harness sampling ShareGPT its own way. Ours drew 303 output tokens per request, upstream 192 (ratio 1.58):

| Metric | This harness | `vllm bench serve` | Δ | Agrees (±5 %) |
|---|---|---|---|---|
| TTFT mean | 146.11 ms | 145.72 ms | +0.3 % | yes |
| TTFT p99 | 385.01 ms | 291.33 ms | +32.2 % | **NO** |
| TPOT mean | 39.06 ms | 37.69 ms | +3.6 % | yes |
| TPOT p99 | 45.67 ms | 49.05 ms | -6.9 % | **NO** |
| E2E mean | 11948.24 ms | 7350.23 ms | +62.6 % | no* |
| Output throughput | 1028.17 tok/s | 685.51 tok/s | +50.0 % | no* |

\* length-sensitive: a workload difference alone moves this metric. The throughput ratio predicted from output length and window alone is 1.494 against a measured 1.500, and each harness's E2E matches its own TTFT + TPOT x (out - 1) to within 0.05 %. The disagreement is the workload, not the code — which is why the matched run above exists.
<!-- END:crossvalidation-table -->

Both harnesses were given the same seed, rate, corpus, endpoint and `ignore_eos`, with
Poisson arrivals stated explicitly on the upstream side rather than left to a default. What
differs between them is precisely what is under test: the dispatch loop, the SSE parsing, the
TTFT definition and the percentile implementation.

---

## 7. The engine axis

SGLang was run as a reduced confirmation sweep, not as a co-equal subject — quantization is the
primary axis and got the GPU-hours. Two observations:

<!-- BEGIN:engine-axis-table -->
| Rate | Checkpoint | vLLM tok/s | SGLang tok/s | vLLM TTFT p95 | SGLang TTFT p95 |
|---|---|---|---|---|---|
| 1 rps | bf16 | 265 | 265 | 168 ms | 143 ms |
| 1 rps | awq-int4 | 286 | 287 | 104 ms | 122 ms |
| 4 rps | bf16 | 1052 | 1026 | 263 ms | 224 ms |
| 4 rps | awq-int4 | 1112 | 1112 | 194 ms | 217 ms |
| 8 rps | bf16 | — | 1965 | — | 2670 ms |
| 8 rps | awq-int4 | 2035 | 2054 | 3288 ms | 1465 ms |
<!-- END:engine-axis-table -->

**Below saturation the engines are indistinguishable.** At 1 and 4 rps the same checkpoint
delivers the same throughput on either engine to within a percent. Whatever the quantization
findings above are, they are not vLLM artifacts.

**At saturation SGLang degrades more gracefully.** At 8 rps `sglang-bf16` remains reportable
where `vllm-bf16` is oversubscribed in every repeat, and `sglang-awq-int4` holds a far lower
TTFT tail than vLLM on the same checkpoint and kernel. This is a scheduling and admission
difference, not a kernel difference — both engines select `awq_marlin`, asserted from their
startup logs. It was not investigated further and is reported as an observation.

**A confound applies to this section specifically.** Both SGLang configurations were measured
with a busy neighbour GPU in the same chassis, while most of the vLLM ladder was not. That
stamp travels with every affected record. Its measured effect across the 123 valid runs is
1.1 °C *cooler* at 7 MHz *higher* clocks for busy-neighbour runs, with zero throttled samples
anywhere — the opposite direction to the feared effect, and small. The comparison is reported
with the asterisk rather than without it.

---

## 8. Limitations

- **One model, one GPU, one context length.** Llama-3.1-8B at `max_model_len=4096` on a single
  A40. Nothing here establishes behaviour at 70B, at long context, or across tensor parallelism.
- **The INT4 decode speedup exceeds what the bandwidth model predicts** and this benchmark does
  not isolate why (§2). The INT8 result, which does fit, is the load-bearing evidence.
- **The quality axis is unresolved.** All six configurations lie inside one another's
  confidence intervals on both task benchmarks (§5). Perplexity separates them; task accuracy
  does not, and the Pareto frontier's quality ordering should not be read as significant.
- **SGLang has no perplexity.** GSM8K and IFEval were measured through SGLang and match vLLM
  on the same checkpoint (0.7316 / 0.7316), but scoring a prompt needs `echo=true` with
  `max_tokens=0`, a vLLM extension SGLang rejects with a 400. SGLang is on the frontier by task
  score; its perplexity column is a gap rather than a guess.
- **FP8 is absent by design, not by omission.** The A40 is sm_86 and has no FP8 tensor cores.
  vLLM will still *load* an FP8 checkpoint on Ampere by dequantizing to FP16 — it appears to
  work, yields weight-only compression, and delivers no compute speedup. Publishing that as
  "FP8 on A40" is the error class this repository exists to avoid.
- **`ignore_eos=True`** forces every configuration to perform identical work, which is what
  makes the comparison fair and simultaneously makes the output lengths less realistic than the
  input lengths. The tradeoff is deliberate and stated in METHODOLOGY.md §2.
- **INT8's ceiling is between 9 and 10 rps.** 9 rps is valid at ~2300 tok/s; 10 and 11 rps are
  oversubscribed. The ladder was extended for INT8 alone because it was the only configuration
  still healthy at 8 rps.
- **The near-knee closed-loop rows were measured on a different physical GPU** from their
  open-loop baseline (§6). Same model, same locked clock, and the same-card comparison below the
  knee shows the same null; the confound cannot produce the sign at the knee, but it is there.
- **The closed-loop exhibit contradicts this repository's stated premise** (§6). The document
  says so where the premise was made rather than quietly rewording it.
- **The cost figure is a public reference price, not an invoice.** The measurement GPU is a lab
  machine. Because every configuration runs on the same GPU, the price is a linear scalar and
  cannot reorder the ranking — which is why no price-sensitivity table appears here.
